...
from kerassurgeon.examples.utils import get_data_generators
...
def save_bottleneck_features():
    ...
    model = inception_v3.InceptionV3(
        include_top=False, weights='imagenet', input_tensor=None, input_shape=None, pooling='avg'
    )
...
def train_top_model():
    ...
    model.compile(
        optimizer=SGD(lr=1e-4, momentum=0.9), loss='categorical_crossentropy', metrics=['accuracy']
    )
...
def tune_model():
    ...
    model.compile(
        loss='categorical_crossentropy', optimizer=SGD(lr=1e-4, momentum=0.9), metrics=['accuracy']
    )
